...
def _dinov3_encode_worker(args):
    """GPU worker for DINOv3 encoding."""
    gpu_id, batch_data, model_name, dtype_str, no_resize = args
    os.environ['CUDA_VISIBLE_DEVICES'] = str(gpu_id)
    ...
    if dtype_str == 'float16':
        ...
        out_dtype = np.float16  # Save as fp16 on disk anyway to keep sizes small
    else:
        out_dtype = np.float32

    results = []
    with torch.inference_mode():
        for idx, image_entry in batch_data:
            try:
                # image_entry.img is HxWx3 uint8 (RGB) by our open_dataset()
                img = PIL.Image.fromarray(image_entry.img, mode='RGB')

                # Keep 256×256 grid intact for ViT patch=16 => 16×16=256 patches (+1 CLS)
                inputs = processor(
                    images=img,
                    return_tensors='pt',
                    do_resize=not no_resize and processor.size is not None,  # typically False for your use
                ...
                # last_hidden_state: [1, 1+N, D] -> CLS at position 0
                tokens = outputs.last_hidden_state.squeeze(0).detach().to('cpu').numpy().astype(out_dtype)
                cls = tokens[0]               # [D]
                # sanity: for 256px, patch16 => 257 tokens
                # but don't hard-assert to allow other sizes/models
                results.append((idx, tokens, cls, image_entry.label))
            except Exception as e:
                print(f"[DINOv3 worker {gpu_id}] error on idx {idx}: {e}")
                results.append(None)
    return results


@cmdline.command(name='encode-dinov3')
@click.option('--model-name',  help='HF model id',                 type=str, default='facebook/dinov3-vit7b16-pretrain-lvd1689m', show_default=True)
@click.option('--source',      help='Input dir or archive',        type=str, required=True)
@click.option('--dest',        help='Output dir or archive',       type=str, required=True)
@click.option('--max-images',  help='Max number of images',        type=int)
@click.option('--gpus',        help='#GPUs (processes)',           type=int, default=8, show_default=True)
@click.option('--batch-size',  help='Images per GPU batch',        type=int, default=100, show_default=True)
@click.option('--dtype',       help='Save dtype',                   type=click.Choice(['float32','float16','bfloat16']), default='float16', show_default=True)
@click.option('--no-resize',   help='Don’t resize inputs',         is_flag=True, default=True, show_default=True)
def encode_dinov3(
    model_name: str,
    source: str,
    dest: str,
    ...
    """
    Precompute DINOv3 tokens (CLS + patches) per image.
    Saves one NPZ per image with arrays:
      - tokens: [1+N, D], where tokens[0] is CLS, tokens[1:] are patches
      - cls:    [D]       (duplicated for convenience)
    """
    PIL.Image.init()
    if dest == '':
    ...
    print(f"[encode-dinov3] Processing {num_files} images across {gpus} GPU workers, batch_size={batch_size} ...")

    with mp.Pool(gpus) as pool:
        gpu_batches: List[List[Tuple[int, ImageEntry]]] = [[] for _ in range(gpus)]

        def _flush(gpu_batches):
            # Build args (only non-empty)
            gpu_args = []
            for gpu_id, gpu_batch in enumerate(gpu_batches):
                if gpu_batch:
                    gpu_args.append((gpu_id, gpu_batch, model_name, dtype, no_resize))

            if not gpu_args:
                return []

            all_results = pool.map(_dinov3_encode_worker, gpu_args)
            flat = []
            for part in all_results:
                flat.extend([x for x in part if x is not None])
            return sorted(flat, key=lambda x: x[0])

        for idx, image in tqdm(enumerate(input_iter), total=num_files, desc="DINOv3 encoding"):
            gpu_id = idx % gpus
            gpu_batches[gpu_id].append((idx, image))

            max_len = max(len(b) for b in gpu_batches)
            if max_len >= batch_size:
                current = _flush(gpu_batches)
                # save current
                for idx_i, tokens, cls, label in current:
                    idx_str = f'{idx_i:08d}'
                    arch_fname = f'{idx_str[:5]}/img-dinov3-{idx_str}.npz'
                    buf = io.BytesIO()
                    # compress to keep disk reasonable; fast enough for training loads
                    np.savez_compressed(buf, tokens=tokens, cls=cls)
                    save_bytes(os.path.join(archive_root_dir, arch_fname), buf.getvalue())
                    labels.append([arch_fname, label] if label is not None else None)
                gpu_batches = [[] for _ in range(gpus)]
